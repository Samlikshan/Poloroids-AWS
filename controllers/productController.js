...
const postEditProducts = async(req,res)=>{
    ...
    try {
        ...
        if (req.body.deleteMainImage) {
            ...
            product.mainImage = req.files.mainImage[0].path.replace("..\\Polaroids\\public",'');
        }
        ...
        for (let i = 0; i < product.additionalImages.length; i++) {
            if (req.body[`deleteImage${i}`]) {
                ...
                product.additionalImages[i] = req.files[`replaceImage${i}`][0].path.replace("..\\Polaroids\\public",'')
            }
        }
        ...
        if (req.files.newAdditionalImages) {
            req.files.newAdditionalImages.forEach(file => {
                product.additionalImages.push(file.path.replace("..\\Polaroids\\public",''));
            });
        }
...
const postAddProduct = async (req,res,next) => {
    ...
    try {
        const newProduct = new Product({
            ...
            mainImage: req.files.mainImage[0].path.replace("..\\Polaroids\\public",''), // Store the main image path
            additionalImages: req.files.additionalImages.map(file => file.path.replace("..\\Polaroids\\public",'')) // Store paths of additional images
        });
